...
> **Note (Antigravity conversion):** this skill originally included supporting files under `references/` alongside its `SKILL.md`. Antigravity does not support bundled skill assets, so those files are not included here. See `hermes-history-ingest/` in the obsidian-wiki repo's `.skills/` directory for the full skill with its supporting files.
